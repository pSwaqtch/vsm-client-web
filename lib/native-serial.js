...
function createNativeSerialTransport(options = {}) {
  ...
  async function executeCommands(commands) {
    for (const commandText of commands || []) {
      const line = String(commandText || '').trim();
      if (!line) {
        continue;
      }

      if (/^sw_reset\b/i.test(line)) {
        await writeOnly(line);
        continue;
      }

      if (/^reg_read\b/i.test(line)) {
        await command(line, (responseLine) => (responseLine ? true : null));
        continue;
      }

      if (/^(reg_write|agc_|bioz_cal_en)\b/i.test(line)) {
        try {
          await command(line, (responseLine) => (responseLine ? true : null));
        } catch (error) {
          if (!/Timed out waiting for response/i.test(String(error && error.message))) {
            throw error;
          }
          await writeOnly(line);
        }
