...
function createSimState() {
  ...
  function buildCfgLines(otherRegisters = []) {
    const lines = [];
  ...
  return {
    ...
    exportCfgFile(type, sillicon, otherRegisters = []) {
      ...
      const lines = buildCfgLines(otherRegisters);

      fs.writeFileSync(filePath, lines.join('\n') + '\n', 'utf8');
      return filePath;
    },

    serializeCfgContent(otherRegisters = []) {
      ensureLoaded();
      return buildCfgLines(otherRegisters).join('\n') + '\n';
    },
    ...
    getRegisterAddresses() {
      ensureLoaded();
      return Object.keys(simRegisters);
    },
    ...
    getLatestPreviewCommand() {
      return clone(previewCommandLog[0] || null);
    },

    getPreviewCommandCount() {
      return previewCommandLog.length;
    },
  };
}
