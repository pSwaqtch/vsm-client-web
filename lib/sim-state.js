...
function createSimState() {
  ...
  return {
    ...
    async writeFieldByOptionValue(field, value) {
      ensureLoaded();
      setFieldBits(field, getOptionKeyByValue(field, value));
      return true;
    },
    ...
    async writeSampleRate(type, sample, sillicon, slot = 'NA') {
      ...
      for (const part of sampleField['Sample Bits']) {
        ...
        setFieldBits(attribute, replaceBits);
      }
      return 'success';
    },
    ...
    async writeSampleRateLoop(type, sample, sillicon, slotList) {
      ...
      for (const slot of slotList || []) {
        await this.writeSampleRate(type, sample, sillicon, slot);
      }
      return 'success';
    },
    ...
    async writeSlotEnable(slots, type) {
      ensureLoaded();
      const field = mapping[`${type}_timeslot_en`];
      setFieldBits(field, getOptionKeyByValue(field, slots));
      return 'success';
    },

    async writeOperationMode(slot, type) {
      ensureLoaded();
      await this.writeFieldByOptionValue(mapping[`${slot}`]['Slot Control'], type);
      return 'success';
    },
    ...
    async writePulse(slot, pulseValue, type) {
      ...
      const width = field.EndBit - field.StartBit + 1;
      setFieldBits(field, dec2bin(pulseValue, width));
      return 'success';
    },
    ...
    async writeDecimateFactor(slot, decimateFactorValue) {
      ...
      const width = field.EndBit - field.StartBit + 1;
      setFieldBits(field, dec2bin(decimateFactorValue, width));
      return 'success';
    },
    ...
    async writeCHEnable(slot, channelName) {
      ...
      setFieldBits(field, getOptionKeyByValue(field, channelName));
      return 'success';
    },
    ...
    async writeTIAGain(slot, channelName, resistanceValue) {
      ...
      setFieldBits(field, getOptionKeyByValue(field, resistanceValue));
      return 'success';
    },
    ...
    async writeDACLEDDC(slot, chName, dacValue, sillicon) {
      ...
      const width = field.EndBit - field.StartBit + 1;
      setFieldBits(field, dec2bin(Math.round(dacValue / Constants.ppgDACStep[sillicon]), width));
      return 'success';
    },
    ...
    async writeLedType(slot, ledType, status) {
      ...
      setFieldBits(enableField, status ? selectedKey : disabledKey);
      if (!status) {
    ...
    async writeLedCurrent(slot, ledType, currentValue, sillicon) {
      ...
      setFieldBits(field, dec2bin(dec, width));
      return 'success';
    },
    ...
    async writeRegisterValue(value) {
      ...
      return this.writeRegister(register, nextValue);
    },
